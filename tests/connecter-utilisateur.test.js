...
describe("Se connecter en utilisateur marion2", () => {
    let page
    ...
    test('Sign up', async () => {
        ...
        await page.type('form[action="/signup"] input[name="username"]', "marion2");
        await page.type('form[action="/signup"] input[name="password"]', "marion2");
        await page.type('form[action="/signup"] input[name="email"]', "marion2@marion.com");
        // vérifier avec un screenshot ce qu'on a renseigné
        await page.screenshot({path: './tests/img/connecter_utilisateur/username_marion2.png'});
        //Enregistrer le compte
        await page.click('form[action="/signup"] input[type="submit"]');
    }, timeout)

    // Se connecter à un compte utilisateur
    test('Se connecter en utilisateur marion2', async () => {
        ...
        await page.type('input[name="username"]', 'marion2');
        await page.type('input[name="password"]', 'marion2');
        ...
        // on vérifie qu'il contient la bonne chaîne de caractères
        expect(htmlAdmin).toContain("marion2");
        //Faire un screenshot de la home page pour l'admin
        await page.screenshot({path: './tests/img/connecter_utilisateur/login_marion2.png'});

    }, timeout)


    test('se deconnecter de marion2', async () => {
        await page.click('.login-name');
        await page.waitForSelector('.dropdown-menu');
        await page.screenshot({path: './tests/img/connecter_utilisateur/avant_logout_marion2.png'});
        await page.evaluate( () => {
            Array
        ...
        await page.screenshot({path: './tests/img/connecter_utilisateur/logout_marion2.png'});
        }, timeout)


    // Supprimer un compte utilisateur
    test('Supprimer marion2', async () => {
        ...
        //Faire un screenshot de la home page pour l'admin
        await page.screenshot({path: './tests/img/connecter_utilisateur/login_admin_connecter_utilisateur.png'});
        ...
        await page.screenshot({path: './tests/img/connecter_utilisateur/dashboard_admin_connecter_utilisateur.png'});
        ...
        await page.screenshot({path: './tests/img/connecter_utilisateur/AdminDashboard_connecter_utilisateur.png'});


        //******************************* Supprimer l'utilisateur marion2 ***********************************
        // Entrer une recherche dans la table Users avec un délai pour laisser s'afficher le résultat
        await page.type('#admin_users_table_filter input[type="search"]', "marion2@marion.com", {delay: 100});
        ...
        await page.screenshot({path: './tests/img/connecter_utilisateur/search_marion2.png'});
    ...
    test('se deconnecter du compte admin', async () => {
        ...
        await page.waitForSelector('.dropdown-menu');
        await page.screenshot({path: './tests/img/connecter_utilisateur/avant_logout_admin.png'});
        await page.evaluate( () => {
            Array
            .from( document.querySelectorAll('.dropdown-menu li a'))
            .filter( el => el.textContent === 'Logout')[0].click();
        ...
        await page.screenshot({path: './tests/img/connecter_utilisateur/logout_admin.png'});
